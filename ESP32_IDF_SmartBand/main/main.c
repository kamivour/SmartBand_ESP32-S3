...
static void sensor_task(void *pvParameters) {
    ...
    while (1) {
        ...
        if (!MPU6050_ReadRaw(&ax_raw, &ay_raw, &az_raw, &gx_raw, &gy_raw, &gz_raw)) {
        ...
        if (last_time == 0) dt = 0.02;  // First iteration
        ...
        if (batLevel > 0) batLevel -= 0.001f;
        
        // Create JSON packet
        char json_buffer[JSON_BUFFER_SIZE];
        int json_len = JSON_CreateSensorPacket(json_buffer, sizeof(json_buffer),
                                              current_time,
                                              pitch, roll, svm,
                                              gx, gy, gz,
                                              (int)batLevel);
